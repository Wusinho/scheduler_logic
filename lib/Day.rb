...
class Day
    ...
    until loop_counter == @daily_turns.size
      ...
      if ids.length != 1
        ...
        combined_ids.each do |combination|
          ...
          worker_1.worker_range.each do |range|
            add_conflicts(@conflictions, worker_1, worker_2, range, @conflicted_hours) if worker_2.worker_range.include?(range)
          end
        end
      end
      ...
      @daily_turns.each do |worker|
        ...
        next unless worker.able_to_work
          ...
          if @range_supervised_hours.include?(pair) && !@conflicted_hours.include?(pair)
  ...
  def resolve_conflicted_hours
    ...
    select_workers_able_to_work.each do |worker|
        @conflicted_hours.each do |conflicted_hour|
          ...
          if worker.worker_range.include?(conflicted_hour)
  ...
  def select_workers_able_to_work
    @daily_turns.find_all { |worker| worker.able_to_work}
  end
